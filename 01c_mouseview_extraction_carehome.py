...
DATAFOLDER = "data_exp_90011-v4"
FILENAME = "data_exp_90011-v4_task-pshj.csv"
FILEFOLDER = "uploads"
...
EXCLUDED = [ \
    # The following "participants" are excluded because they are test runs
    ...
    "R_UxOS2Wih7lC9L8t", \
    "R_3O2ENYLRRFa3p0D", \
    # Real person, but second attempt (thought first didn't come through).
    "R_1M3UDpwo9E1ihIR", \
    ]
                ...
                for i in range(NTRIALSPERSTIM):
                    for j, stim in enumerate(["neutral", "affective"]):
                        for stim_nr in range(len(STIMULI[con])):
                            ...
                            if "_" in con:
                                line = [participant, "carehome", environment, \
                                    disgust_type, i, stim, stim_nr, d]
                            else:
                                line = [participant, "carehome", con, i, \
                                    stim, stim_nr, d]
                            # Write to file.
                            f.write("\n" + ",".join(map(str, line)))
...
# Save the figure.
fig.savefig(os.path.join(OUTDIR, "fig-03_carehome_disgust_avoidance.png"))
pyplot.close(fig)
