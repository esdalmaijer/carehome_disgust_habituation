...
FORMULA = { \
    "combined": { \
        "dwell": [ \
            "dwell ~ group + stimulus + environment + disgust_type + repetition + " \
                + "group*stimulus*environment*disgust_type*repetition", \
            "dwell ~ group + stimulus + environment + disgust_type + " \
                + "group*stimulus*environment*disgust_type", \
            "dwell ~ group + stimulus + environment + repetition + " \
                + "group*stimulus*environment*repetition", \
            "dwell ~ group + stimulus + disgust_type + repetition + " \
                + "group*stimulus*disgust_type*repetition", \
            "dwell ~ group + stimulus + environment + " \
                + "group*stimulus*environment", \
            "dwell ~ group + stimulus + disgust_type + " \
                + "group*stimulus*disgust_type", \
            ...
            "dwell ~ stimulus + environment + disgust_type + repetition + " \
                + "stimulus*environment*disgust_type*repetition", \
            "dwell ~ stimulus + environment + disgust_type + " \
                + "stimulus*environment*disgust_type", \
            "dwell ~ stimulus + environment + repetition + " \
                + "stimulus*environment*repetition", \
            "dwell ~ stimulus + disgust_type + repetition + " \
                + "stimulus*disgust_type*repetition", \
            "dwell ~ stimulus + environment + " \
                + "stimulus*environment", \
            "dwell ~ stimulus + disgust_type + " \
                + "stimulus*disgust_type", \
...
STANDARDISE_THESE = ["dwell", "repetition"]
...
for datatype in FORMULA[EXPNAME].keys():

    # Select data to load.
    if datatype == "dwell":
        file_names = ["{}_dwell_means_long".format(EXPNAME)]
    ...
    for fname in file_names:
        ...
        if IMPUTE_MISSING:
            ...
            if datatype == "rating":
                value = "rating"
                within_columns = ["stimulus", "moment"]
                max_missing_prop = 0.0
            elif datatype == "dwell":
                value = "dwell"
                within_columns = ["environment", "disgust_type", "repetition", "stimulus", "stim_nr"]
        ...
        with open(fpath, "w") as f:
            ...
            for i, formula in enumerate(FORMULA[EXPNAME][datatype]):
